...
describe("setup command", () => {
  ...
  it("should inform the user if Starwind Pro is already configured", async () => {
    ...
    mockHasStarwindProAuthConfig.mockReturnValue(true);
    mockCheckStarwindProEnv.mockResolvedValue(true);
    await setup();
    expect(mockLog.info).toHaveBeenCalledWith(
      expect.stringContaining("Starwind Pro paid authorization is already configured"),
    );
    expect(mockSetupStarwindProConfig).not.toHaveBeenCalled();
    expect(mockSetupStarwindProEnv).not.toHaveBeenCalled();
  });

  it("should add Pro auth when only a partial Pro registry is configured", async () => {
    mockGetConfig.mockResolvedValue({
      $schema: "https://starwind.dev/config-schema.v2.json",
      version: 2,
      framework: "astro",
  ...
  it("should handle errors during setup", async () => {
    ...
    await expect(setup()).rejects.toThrow("process.exit called");
    expect(mockLog.error).toHaveBeenCalledWith("Failed to update starwind.config.json");
