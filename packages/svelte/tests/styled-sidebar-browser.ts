...
import S from "./sidebar/index.js";import {CollapsibleRoot,CollapsibleTrigger,CollapsiblePanel} from "@starwind-ui/svelte/collapsible";import {useSidebarContext} from "@starwind-ui/svelte/sidebar";import {createAttachmentKey,type Attachment} from "svelte/attachments";
...
<CollapsibleRoot><S.MenuItem><CollapsibleTrigger>{#snippet child({props})}<S.MenuButton {...props} tooltip="Models" data-models>Models</S.MenuButton>{/snippet}</CollapsibleTrigger><CollapsiblePanel>Model choices</CollapsiblePanel></S.MenuItem></CollapsibleRoot>
...
const checkHeading=async()=>{const heading=sidebar().querySelector('[data-models]');assert(!heading.disabled&&!heading.hasAttribute("data-disabled"),"tooltip must not disable composed Sidebar heading");const before=heading.getAttribute("aria-expanded");await click(heading);assert(heading.getAttribute("aria-expanded")!==before,"Sidebar heading toggles its Collapsible");};
...
assert(!popup().open,"mobile initially closed");assert(q("#main").querySelector('[data-sidebar="mobile"]')!==sheet(),"nested first-descendant discovery is exercised");await click(q('[data-trigger="outer"]'));assert(app.snapshot().mobileOpen&&popup().open&&!app.snapshot().innerMobile,"owned mobile Sheet bridge contains nested discovery");const before=app.snapshot().events.length;sheet().addEventListener("starwind:open-change",event=>event.preventDefault(),{once:true});await click(close());assert(app.snapshot().mobileOpen&&popup().open&&app.snapshot().events.length===before,"canceled Sheet request retains accepted model");await click(close());assert(!app.snapshot().mobileOpen&&!popup().open&&app.snapshot().events.length===before+1&&app.snapshot().events.at(-1)==="mobile:false:mobile-sheet","accepted Sheet output reports one accepted mobile close");app.command({mobileOpen:true});await finish();app.command({mobileOpen:false});flushSync();await tick();app.command({mobileOpen:true});await finish();popup().dispatchEvent(new Event("transitionend"));await finish();assert(app.snapshot().mobileOpen&&popup().open,"newer open survives old close completion");await checkHeading();result.sheet=true;
}else{
