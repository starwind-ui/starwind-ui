...
const motionStyle=document.createElement('style');motionStyle.textContent='[data-case="bound"] [data-panel] { opacity: 1; transition: opacity 180ms linear; } [data-case="bound"] [data-panel][data-starting-style], [data-case="bound"] [data-panel][data-ending-style] { opacity: 0; }';document.head.append(motionStyle);const outgoing=root('bound').querySelector('[data-panel="a"]');equal(outgoing.hasAttribute('data-starting-style'),false,'initial animation suppressed');getComputedStyle(outgoing).opacity;cases.bound.setModel('b');await settle();equal(outgoing.hidden,false,'outgoing retained through rerender');equal(outgoing.inert,true,'outgoing inert');equal(tab('bound','b').tabIndex,0,'external entry updated');await new Promise(resolve=>setTimeout(resolve,240));equal(outgoing.hidden,true,'exit completed');tab('bound','a').focus();cases.bound.setModel('a');await settle();cases.bound.setModel('b');await settle();equal(document.activeElement,tab('bound','a'),'inside focus retained');equal(tab('bound','a').tabIndex,0,'inside keyboard position retained');motionStyle.remove();document.activeElement.blur();cases.bound.setModel('a');await settle();
