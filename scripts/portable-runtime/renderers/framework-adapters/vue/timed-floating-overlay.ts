...
<template>
  <Teleport :to="props.container" :disabled="props.disabled || !root.mounted.value">
    <${facts.parts.portal.defaultElement} ref="element" v-bind="attrs" ${facts.attrs.portal} data-sw-part="${facts.parts.portal.name}"><slot /></${facts.parts.portal.defaultElement}>
  </Teleport>
</template>
`;
}
...
<template><${part.defaultElement} ref="element" v-bind="attrs" ${discovery} data-sw-part="${part.name}" ${extras}><slot /></${part.defaultElement}></template>
`;
}
