...
export function specializeSvelteStyledSheet(
...
> {
  ...
  const native = ["SheetHeader", "SheetFooter"].includes(component.exportName);
  if (!part && !native) return fail("missing Sheet part owner");
  for (const entry of component.imports.filter((entry) => supportsSvelteScope(entry.targetScopes)))
    if (!entry.svg) fail(`unsupported Sheet import "${entry.source}"`);
  component.imports = [];
  if (part === "Trigger" || part === "Close") {
    ...
    if (
      ...
      return fail("Sheet requires its child composition branch");
  ...
  function wire(nodes: StyledOutputRenderNode[]): void {
    for (const node of nodes) {
      ...
      if (node.type === "primitive" && node.component === "drawer" && node.part === part) {
        owners++;
        if (part === "Root") {
          for (const name of ["onOpenChange", "onCloseComplete"]) {
            ...
            if (!attr) fail(`Sheet is missing its ${name} forwarding contract`);
      ...
      if (native && node.type === "element" && node.tag === "div") {
        ...
        if (!spread) fail("Sheet native part requires attribute forwarding");
  ...
  if (owners !== 1) fail("Sheet requires one semantic owner");
  ...
  const nativeType = native
    ...
    : `ComponentProps<typeof Drawer${part}>`;
  return {
    imports: [
      ...
      ...(native
        ...
        : [{ source: sveltePrimitiveImport("drawer", options), names: [`Drawer${part}`] }]),
