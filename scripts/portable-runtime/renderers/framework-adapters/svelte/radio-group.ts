...
export function printSvelteRadioGroupIndex(file: AdapterIndexFile): AdapterPrintedFile {
  const family = file.family;
  if (
    family?.kind !== "grouped-value-control" ||
    family.facts.runtime.factory !== "createRadioGroup"
  )
    throw new TypeError("Svelte Radio Group requires its grouped-value facts.");
  const { facts } = family;
  return {
    path: file.path,
...
export function printSvelteRadioGroupComponent(file: AdapterComponentFile): AdapterPrintedFile {
