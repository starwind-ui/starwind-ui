...
function renderSolidSelectPortal(spec: SelectSpecializedAdapterSpec): string {
  ...
  return `/* ${SELECT_FIXTURE_COMMENT} */\nimport { reportPortalPlacement, resolvePortalPlacement } from "${spec.root.runtimeImportSource}";\nimport { createEffect, createSignal, onCleanup, onMount, Show, splitProps } from "solid-js";\nimport type { JSX } from "solid-js";\nimport { Portal } from "solid-js/web";\n\nexport type SelectPortalProps = Omit<JSX.HTMLAttributes<HTMLDivElement>, "children"> & {\n  container?: string | HTMLElement;\n  disabled?: boolean;\n};\n\nexport function SelectPortal(allProps: SelectPortalProps) {\n  const [local, rest] = splitProps(allProps, ["children", "container", "disabled"]);\n  const [mounted, setMounted] = createSignal(false);\n  const [placementRevision, setPlacementRevision] = createSignal(0);\n  const [portalContainer, setPortalContainer] = createSignal<HTMLDivElement>();\n  const [portalTarget, setPortalTarget] = createSignal<HTMLElement>();\n  const [wrapper, setWrapper] = createSignal<HTMLDivElement>();\n  let inlineReference: HTMLElement | undefined;\n\n  onMount(() => setMounted(true));\n  onCleanup(() => {\n    const element = wrapper();\n    if (element) reportPortalPlacement(element, null);\n  });\n\n  createEffect(() => {\n    placementRevision();\n    const element = wrapper();\n    if (!element) return;\n    inlineReference ??= element.parentElement ?? element;\n    onCleanup(() => reportPortalPlacement(element, null));\n\n    if (!mounted() || local.disabled) {\n      setPortalTarget(undefined);\n      return;\n    }\n\n    const requestedContainer = local.container;\n    const requestedTarget = resolvePortalPlacement(element, {\n      container: requestedContainer,\n      disabled: false,\n      mode: "framework",\n      reference: inlineReference,\n    }).target;\n    setPortalTarget(requestedTarget);\n\n    const nativeContainer = portalContainer();\n    if (!nativeContainer || nativeContainer.parentElement !== requestedTarget) {\n      reportPortalPlacement(element, { ready: false, target: requestedTarget });\n      if (nativeContainer) {\n        const observer = new MutationObserver(() => {\n          if (nativeContainer.parentElement === requestedTarget) {\n            setPlacementRevision((revision) => revision + 1);\n          }\n        });\n        observer.observe(requestedTarget.ownerDocument, { childList: true, subtree: true });\n        onCleanup(() => observer.disconnect());\n      }\n      return;\n    }\n\n    const readinessTarget = resolvePortalPlacement(element, {\n      container: nativeContainer,\n      disabled: false,\n      mode: "framework",\n      reference: inlineReference,\n    }).target;\n    reportPortalPlacement(element, { ready: false, target: readinessTarget });\n    if (element.parentElement === readinessTarget) {\n      reportPortalPlacement(element, { ready: true, target: readinessTarget });\n    }\n  });\n\n  const publicWrapper = () => (\n    <div\n      ref={setWrapper}\n      ${attrs.portal}\n      data-placement="pending"\n      data-sw-portal-placement="framework"\n      {...rest}\n    >\n      {local.children}\n    </div>\n  );\n\n  return (\n    <Show when={mounted() && portalTarget() && !local.disabled} fallback={publicWrapper()}>\n      {/* Solid Portal owns one internal div. It is not a Starwind public part. */}\n      <Portal mount={portalTarget()!} ref={setPortalContainer}>\n        {publicWrapper()}\n      </Portal>\n    </Show>\n  );\n}\n\nexport default SelectPortal;\n`;
}
