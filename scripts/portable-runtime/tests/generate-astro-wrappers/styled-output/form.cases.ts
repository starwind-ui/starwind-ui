...
export async function assertAstroStyledFormOutput(outputRoot: string): Promise<void> {
  const select = await readGeneratedFile(outputRoot, "select/Select.astro");
  const selectTrigger = await readGeneratedFile(outputRoot, "select/SelectTrigger.astro");
  const selectContent = await readGeneratedFile(outputRoot, "select/SelectContent.astro");
  ...
  const combobox = await readGeneratedFile(outputRoot, "combobox/Combobox.astro");
  const comboboxInputGroup = await readGeneratedFile(
    outputRoot,
    "combobox/ComboboxInputGroup.astro",
  );
  ...
  const comboboxContent = await readGeneratedFile(outputRoot, "combobox/ComboboxContent.astro");
  ...
  const checkbox = await readGeneratedFile(outputRoot, "checkbox/Checkbox.astro");
  ...
  const checkboxGroup = await readGeneratedFile(outputRoot, "checkbox-group/CheckboxGroup.astro");
  ...
  const radioGroup = await readGeneratedFile(outputRoot, "radio-group/RadioGroup.astro");
  const radioGroupItem = await readGeneratedFile(outputRoot, "radio-group/RadioGroupItem.astro");
  ...
  const input = await readGeneratedFile(outputRoot, "input/Input.astro");
  ...
  const inputGroup = await readGeneratedFile(outputRoot, "input-group/InputGroup.astro");
  const inputGroupAddon = await readGeneratedFile(outputRoot, "input-group/InputGroupAddon.astro");
  const inputGroupButton = await readGeneratedFile(
    outputRoot,
    "input-group/InputGroupButton.astro",
  );
  const inputGroupInput = await readGeneratedFile(outputRoot, "input-group/InputGroupInput.astro");
  const inputGroupText = await readGeneratedFile(outputRoot, "input-group/InputGroupText.astro");
  const inputGroupTextarea = await readGeneratedFile(
    outputRoot,
    "input-group/InputGroupTextarea.astro",
  );
  ...
  const fieldContent = await readGeneratedFile(outputRoot, "field/FieldContent.astro");
  ...
  const fieldGroup = await readGeneratedFile(outputRoot, "field/FieldGroup.astro");
  ...
  const fieldLegend = await readGeneratedFile(outputRoot, "field/FieldLegend.astro");
  const fieldSeparator = await readGeneratedFile(outputRoot, "field/FieldSeparator.astro");
  const fieldSet = await readGeneratedFile(outputRoot, "field/FieldSet.astro");
  const fieldTitle = await readGeneratedFile(outputRoot, "field/FieldTitle.astro");
  ...
  const inputOtp = await readGeneratedFile(outputRoot, "input-otp/InputOtp.astro");
  ...
  const progress = await readGeneratedFile(outputRoot, "progress/Progress.astro");
  ...
  const slider = await readGeneratedFile(outputRoot, "slider/Slider.astro");
  ...
  const switchComponent = await readGeneratedFile(outputRoot, "switch/Switch.astro");
  ...
  expect(select).toContain('SelectPrimitive from "../primitives/astro/select"');
  ...
  expect(combobox).toContain('ComboboxPrimitive from "../primitives/astro/combobox"');
  ...
  expect(comboboxTrigger).toContain('<slot name="icon">');
  ...
  expect(checkbox).toContain('CheckboxPrimitive from "../primitives/astro/checkbox"');
  expect(checkbox).toContain('import Check from "@tabler/icons/outline/check.svg"');
  ...
  expect(checkbox).toContain('Omit<HTMLAttributes<"span">, "defaultChecked" | "id" | "onChange">');
  ...
  expect(checkbox).not.toContain("@starwind-ui/runtime");
  ...
  expect(checkboxGroup).toContain(
    'CheckboxGroupPrimitive from "../primitives/astro/checkbox-group"',
  );
  ...
  expect(checkboxGroup).not.toContain("@starwind-ui/runtime");
  ...
  expect(input).toContain('InputPrimitive from "../primitives/astro/input"');
  expect(input).toContain("<InputPrimitive.Root");
  expect(input).toContain('extends Omit<HTMLAttributes<"input">');
  expect(input).toContain('"children" | "defaultValue" | "size" | "value"');
  expect(input).toContain("VariantProps<typeof input>");
  ...
  expect(input).not.toContain("@starwind-ui/runtime");
  ...
  expect(inputGroup).toContain('type Props = HTMLAttributes<"div">;');
  ...
  expect(inputGroupButton).toContain('import type { ComponentProps } from "astro/types";');
  expect(inputGroupButton).toContain('import { Button } from "../button";');
  expect(inputGroupButton).toContain('Omit<ComponentProps<typeof Button>, "size">');
  ...
  expect(inputGroupInput).toContain("ComponentProps<typeof Input>");
  ...
  expect(inputGroupTextarea).toContain("ComponentProps<typeof Textarea>");
  ...
  expect(form).toContain('FormPrimitive from "../primitives/astro/form"');
  ...
  expect(field).toContain('FieldPrimitive from "../primitives/astro/field"');
  ...
  expect(fieldSet).toContain('FieldsetPrimitive from "../primitives/astro/fieldset"');
  ...
  expect(fieldSeparator).toContain("const hasContent = Astro.slots.has");
  ...
  expect(fieldSeparator).toContain('data-content={hasContent ? "true" : "false"}');
  expect(fieldSeparator).toContain("<Separator");
  expect(fieldSeparator).toContain('class="absolute inset-0 top-1/2"');
  ...
  expect(inputOtpSeparator).toContain('slot name="icon"');
  ...
  expect(progress).toContain('ProgressPrimitive from "../primitives/astro/progress"');
  ...
  expect(progress).toContain("value == null || !Number.isFinite(Number(value))");
  expect(progress).toContain("Math.min(Math.max(Number(value), normalizedMin), normalizedMax)");
  expect(progress).toContain("const normalizedMin = Math.min(boundedMin, boundedMax);");
  expect(progress).toContain("const normalizedMax = Math.max(boundedMin, boundedMax);");
  expect(progress).toContain("normalizedMax === normalizedMin");
  expect(progress).toContain("(progressValue - normalizedMin) / (normalizedMax - normalizedMin)");
  expect(progress).toContain("max={normalizedMax}");
  expect(progress).toContain("min={normalizedMin}");
  expect(progress).toContain("value={progressValue}");
  expect(progress).toContain('data-slot="progress"');
  expect(progress).not.toContain("@starwind-ui/runtime");
  ...
  expect(progressVariants).toContain("data-instant:transition-none");
  expect(progressVariants).toContain("motion-reduce:transition-none");
  expect(progressIndex).toContain("export default Progress;");
  expect(progressIndex).not.toContain("Root: Progress");
  expect(slider).toContain('SliderPrimitive from "../primitives/astro/slider"');
  ...
  expect(slider).toContain("defaultValue?: number | number[];");
  ...
  expect(slider).not.toContain("@starwind-ui/runtime");
  ...
  expect(switchComponent).toContain('SwitchPrimitive from "../primitives/astro/switch"');
  ...
  expect(switchComponent).toContain("style={switchStyle}");
  expect(switchComponent).toContain("style={thumbStyle}");
  ...
  expect(switchComponent).not.toContain("@starwind-ui/runtime");
