...
export async function assertAstroStyledFoundationOutput(outputRoot: string): Promise<void> {
  ...
  const separator = await readGeneratedFile(outputRoot, "separator/Separator.astro");
  ...
  const scrollArea = await readGeneratedFile(outputRoot, "scroll-area/ScrollArea.astro");
  ...
  const scrollBar = await readGeneratedFile(outputRoot, "scroll-area/ScrollBar.astro");
  const scrollAreaThumb = await readGeneratedFile(outputRoot, "scroll-area/ScrollAreaThumb.astro");
  ...
  const kbd = await readGeneratedFile(outputRoot, "kbd/Kbd.astro");
  const kbdGroup = await readGeneratedFile(outputRoot, "kbd/KbdGroup.astro");
  ...
  const textarea = await readGeneratedFile(outputRoot, "textarea/Textarea.astro");
  ...
  const skeleton = await readGeneratedFile(outputRoot, "skeleton/Skeleton.astro");
  ...
  const spinner = await readGeneratedFile(outputRoot, "spinner/Spinner.astro");
  ...
  const prose = await readGeneratedFile(outputRoot, "prose/Prose.astro");
  ...
  const card = await readGeneratedFile(outputRoot, "card/Card.astro");
  const cardHeader = await readGeneratedFile(outputRoot, "card/CardHeader.astro");
  const cardTitle = await readGeneratedFile(outputRoot, "card/CardTitle.astro");
  const cardDescription = await readGeneratedFile(outputRoot, "card/CardDescription.astro");
  const cardContent = await readGeneratedFile(outputRoot, "card/CardContent.astro");
  const cardFooter = await readGeneratedFile(outputRoot, "card/CardFooter.astro");
  const cardAction = await readGeneratedFile(outputRoot, "card/CardAction.astro");
  ...
  const breadcrumb = await readGeneratedFile(outputRoot, "breadcrumb/Breadcrumb.astro");
  const breadcrumbList = await readGeneratedFile(outputRoot, "breadcrumb/BreadcrumbList.astro");
  const breadcrumbItem = await readGeneratedFile(outputRoot, "breadcrumb/BreadcrumbItem.astro");
  const breadcrumbLink = await readGeneratedFile(outputRoot, "breadcrumb/BreadcrumbLink.astro");
  const breadcrumbPage = await readGeneratedFile(outputRoot, "breadcrumb/BreadcrumbPage.astro");
  const breadcrumbSeparator = await readGeneratedFile(
    outputRoot,
    "breadcrumb/BreadcrumbSeparator.astro",
  );
  const breadcrumbEllipsis = await readGeneratedFile(
    outputRoot,
    "breadcrumb/BreadcrumbEllipsis.astro",
  );
  ...
  const alert = await readGeneratedFile(outputRoot, "alert/Alert.astro");
  const alertTitle = await readGeneratedFile(outputRoot, "alert/AlertTitle.astro");
  const alertDescription = await readGeneratedFile(outputRoot, "alert/AlertDescription.astro");
  ...
  expect(badge).not.toContain("@starwind-ui/runtime");
  ...
  expect(label).not.toContain("@starwind-ui/runtime");
  ...
  expect(separator).toContain('Omit<HTMLAttributes<"div">, "role" | "aria-orientation">');
  ...
  expect(scrollArea).toContain('ScrollAreaPrimitive from "../primitives/astro/scroll-area"');
  ...
  expect(scrollArea).toContain("<slot />");
  expect(scrollArea).toContain('<slot name="scrollbar">');
  ...
  expect(scrollArea).toContain("viewportClass");
  ...
  expect(kbd).toContain("class={kbd({ class: className })}");
  expect(kbd).toContain('data-slot="kbd"');
  expect(kbdGroup).toContain("<kbd");
  expect(kbdGroup).toContain("data-sw-kbd-group");
  expect(kbdGroup).toContain("class={kbdGroup({ class: className })}");
  expect(kbdGroup).toContain("<slot />");
  expect(kbdGroup).toContain('data-slot="kbd-group"');
  expect(kbd).not.toContain("@starwind-ui/runtime");
  ...
  expect(textarea).toContain('Omit<HTMLAttributes<"textarea">, "children">');
  ...
  expect(textarea).not.toContain("ref={ref}");
  expect(textarea).not.toContain("@starwind-ui/runtime");
  ...
  expect(skeleton).toContain('Omit<HTMLAttributes<"div">, "children">');
  expect(skeleton).toContain("skeleton({ class: className })");
  expect(skeleton).toContain('data-slot="skeleton"');
  expect(skeleton).not.toContain("@starwind-ui/runtime");
  ...
  expect(prose).toContain('type Props = HTMLAttributes<"div">;');
  ...
  expect(themeToggle).toContain('Omit<HTMLAttributes<"button">');
  ...
  expect(themeToggle).toContain("data-slot={dataSlot}");
  ...
  expect(themeToggle).toContain("const setupThemeController = () =>");
  expect(themeToggle).toContain("initThemeController();");
  expect(themeToggle).toContain(
    'document.addEventListener("astro:after-swap", setupThemeController)',
  );
  expect(themeToggle).toContain('document.addEventListener("starwind:init", setupThemeController)');
  ...
  expect(sidebarProvider).toContain('SidebarPrimitive from "../primitives/astro/sidebar"');
  ...
  expect(sidebarProvider).toContain("providerStyle");
  ...
  expect(sidebarTrigger).toContain(
    'import LayoutSidebar from "@tabler/icons/outline/layout-sidebar.svg"',
  );
  expect(sidebarTrigger).toContain('SidebarPrimitive from "../primitives/astro/sidebar"');
  ...
  expect(sidebarMenuButton).toContain('SidebarPrimitive from "../primitives/astro/sidebar"');
  ...
  expect(toastTemplate).toContain('type Props = HTMLAttributes<"div"> & {');
  expect(toastTemplate).not.toContain('HTMLAttributes<"template">');
  ...
  expect(card).not.toContain("@starwind-ui/runtime");
  ...
  expect(breadcrumbLink).toContain("asChild = false");
  expect(breadcrumbLink).toContain("asChild ?");
  expect(breadcrumbLink).toContain("<slot />");
  ...
  expect(breadcrumbEllipsis).toContain('import Dots from "@tabler/icons/outline/dots.svg"');
  expect(breadcrumbEllipsis).toContain('<slot name="icon">');
  expect(breadcrumbEllipsis).toContain("<Dots />");
  expect(breadcrumbEllipsis).toContain('<span class="sr-only">More</span>');
  expect(breadcrumb).not.toContain("@starwind-ui/runtime");
  ...
  expect(alert).toContain('type Props = HTMLAttributes<"div"> & VariantProps<typeof alert>;');
  ...
  expect(alert).not.toContain("@starwind-ui/runtime");
