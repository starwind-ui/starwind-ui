...
export async function verifyReactDialogCases({ page }) {
  ...
  if (
    ...
    throw new Error(
      `Expected closing dialog and backdrop to remain visible during exit animation, got ${JSON.stringify(
        {
          closingDialog,
  ...
  await page.getByRole("heading", { name: "React nested parent" }).waitFor();
  await page.getByRole("button", { name: "Open nested level 1" }).click();
  await page.getByRole("heading", { name: "React nested level 1" }).waitFor();
  await page.getByRole("button", { name: "Open nested level 2" }).click();
  await page.getByRole("heading", { name: "React nested level 2" }).waitFor();
  ...
  const nestedDialogStack = await page.evaluate(() => {
    ...
    return {
      levelOne: readDialog("#react-runtime-nested-dialog-level-one"),
      levelOneClose: readControl(
        "#react-runtime-nested-dialog-level-one-close",
        "data-sw-dialog-close",
      ),
      levelOneTrigger: readControl(
        "#react-runtime-nested-dialog-level-one-trigger",
        "data-sw-dialog-trigger",
      ),
      levelTwo: readDialog("#react-runtime-nested-dialog-level-two"),
      levelTwoClose: readControl(
        "#react-runtime-nested-dialog-level-two-close",
        "data-sw-dialog-close",
      ),
      levelTwoTrigger: readControl(
        "#react-runtime-nested-dialog-level-two-trigger",
        "data-sw-dialog-trigger",
      ),
      openCount: document.querySelectorAll("dialog[open]").length,
      parent: readDialog("#react-runtime-nested-dialog-parent"),
      parentClose: readControl("#react-runtime-nested-dialog-parent-close", "data-sw-dialog-close"),
      parentTrigger: readControl(
        "#react-runtime-nested-dialog-parent-trigger",
        "data-sw-dialog-trigger",
      ),
    };
  });
  ...
  if (
    ...
    throw new Error(
      `Expected React nested dialogs to keep all three layers open, got ${JSON.stringify(
        nestedDialogStack,
      )}.`,
    );
  }

  await page.locator("#react-runtime-nested-dialog-level-one-close").evaluate((button) => {
  ...
  if (
    ...
    throw new Error(
      `Expected React ancestor close to be ignored while level 2 is topmost, got ${JSON.stringify(
  ...
  if (
    ...
    throw new Error(
      `Expected React nested dialog sizing to update immediately while level 2 exits, got ${JSON.stringify(
  ...
  const nestedDialogAfterLevelTwoClose = await page.evaluate(() => {
    ...
    return {
      levelOneOpen: readOpen("#react-runtime-nested-dialog-level-one"),
      levelTwoOpen: readOpen("#react-runtime-nested-dialog-level-two"),
      openCount: document.querySelectorAll("dialog[open]").length,
      parentNestedCount:
        document
          .querySelector("#react-runtime-nested-dialog-parent [data-slot='dialog-content']")
          ?.style.getPropertyValue("--nested-dialogs") ?? null,
      parentOpen: readOpen("#react-runtime-nested-dialog-parent"),
      restoredFocusId:
  ...
  if (
    ...
    nestedDialogAfterLevelTwoClose.restoredFocusId !==
      "react-runtime-nested-dialog-level-two-trigger"
  ) {
    throw new Error(
      `Expected Escape to close only React level 2 and restore focus, got ${JSON.stringify(
        nestedDialogAfterLevelTwoClose,
      )}.`,
    );
  }

  await page.locator("#react-runtime-nested-dialog-level-one-close").click();
