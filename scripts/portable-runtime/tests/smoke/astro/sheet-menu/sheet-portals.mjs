export async function verifyAstroSheetPortalCases({ page }) {
  const sheetDropdownState = await page.evaluate(() => {
    const sheetContent = document.querySelector(
      "#runtime-sheet-dropdown-sheet [data-slot='sheet-content']",
    );
    const content = document.querySelector("#runtime-sheet-dropdown-content");
    const floatingRoot = content?.closest("[data-floating-root]");
    const sheetTrigger = document.querySelector("#runtime-sheet-dropdown-sheet-trigger");
    const root = document.querySelector("#runtime-sheet-dropdown");
    const trigger = document.querySelector("#runtime-sheet-dropdown-trigger");
    ...
    return {
      ...
      closeClassName:
        document.querySelector("#runtime-sheet-dropdown-close") instanceof HTMLElement
          ? document.querySelector("#runtime-sheet-dropdown-close").className
          : null,
      closeDataSlot: document
        .querySelector("#runtime-sheet-dropdown-close")
  ...
  if (
    ...
    sheetDropdownState.triggerDataSlot !== "button" ||
    ...
    throw new Error(
      `Expected Astro Dropdown inside Sheet to portal through the Dialog-owned top-layer host, got ${JSON.stringify(
        sheetDropdownState,
      )}.`,
    );
  }

  await page.locator("#runtime-sheet-dropdown-sub-trigger").focus();
  await page.keyboard.press("ArrowRight");
  await page.locator("#runtime-sheet-dropdown-sub-content").waitFor({ state: "visible" });
  await page.waitForFunction(() => document.activeElement?.id === "runtime-sheet-dropdown-email");
  const sheetDropdownSubmenuState = await page.evaluate(() => {
    const sheetContent = document.querySelector(
      "#runtime-sheet-dropdown-sheet [data-slot='sheet-content']",
    );
    const menuContent = document.querySelector("#runtime-sheet-dropdown-content");
    const submenuContent = document.querySelector("#runtime-sheet-dropdown-sub-content");
    const floatingRoot = submenuContent?.closest("[data-floating-root]");
    const trigger = document.querySelector("#runtime-sheet-dropdown-sub-trigger");
  ...
  if (
    ...
    throw new Error(
      `Expected Astro Dropdown submenu inside Sheet to stay in the Dialog-owned top-layer host, got ${JSON.stringify(
        sheetDropdownSubmenuState,
      )}.`,
    );
  }

  await page.locator("#runtime-sheet-dropdown-email").click();
  await page.waitForFunction(() => {
    const content = document.querySelector("#runtime-sheet-dropdown-content");
    const submenu = document.querySelector("#runtime-sheet-dropdown-sub-content");
  ...
  await page.getByRole("button", { name: "Close sheet menu" }).click();
  await page.waitForFunction(() => document.querySelectorAll("dialog[open]").length === 0);

  await page.getByRole("button", { name: "Open sheet popover" }).click();
  await page.getByRole("heading", { name: "Sheet popover portal" }).waitFor();
  await page.locator("#runtime-sheet-popover-details-trigger").focus();
  await page.keyboard.press("Enter");
  await page.locator("#runtime-sheet-popover-details-content").waitFor({ state: "visible" });
  const sheetPopoverState = await page.evaluate(() => {
    const sheetContent = document.querySelector(
      "#runtime-sheet-popover [data-slot='sheet-content']",
    );
    const content = document.querySelector("#runtime-sheet-popover-details-content");
    const floatingRoot = content?.closest("[data-floating-root]");
    const sheetTrigger = document.querySelector("#runtime-sheet-popover-trigger");
    const root = document.querySelector("#runtime-sheet-popover-details");
    const trigger = document.querySelector("#runtime-sheet-popover-details-trigger");
    const close = document.querySelector("#runtime-sheet-popover-close");
  ...
  if (
    ...
    sheetPopoverState.triggerDataSlot !== "button" ||
    ...
    throw new Error(
      `Expected Astro Popover inside Sheet to portal through the Dialog-owned top-layer host, got ${JSON.stringify(
        sheetPopoverState,
      )}.`,
    );
  }

  await page.locator("#runtime-sheet-popover-nested-trigger").focus();
  await page.keyboard.press("Enter");
  await page.locator("#runtime-sheet-popover-nested-content").waitFor({ state: "visible" });
  const sheetNestedPopoverState = await page.evaluate(() => {
    const sheetContent = document.querySelector(
      "#runtime-sheet-popover [data-slot='sheet-content']",
    );
    const parentContent = document.querySelector("#runtime-sheet-popover-details-content");
    const nestedContent = document.querySelector("#runtime-sheet-popover-nested-content");
    const floatingRoot = nestedContent?.closest("[data-floating-root]");
    const trigger = document.querySelector("#runtime-sheet-popover-nested-trigger");
    ...
    return {
      ...
      nestedRootContains:
        document.querySelector("#runtime-sheet-popover-nested") instanceof HTMLElement &&
        nestedContent instanceof HTMLElement
          ? document.querySelector("#runtime-sheet-popover-nested").contains(nestedContent)
          : null,
  ...
  if (
    ...
    throw new Error(
      `Expected nested Astro Popover inside Sheet to stay in the Dialog-owned top-layer host, got ${JSON.stringify(
        sheetNestedPopoverState,
      )}.`,
    );
  }

  await page.locator("#runtime-sheet-popover-close").click();
  await page.waitForFunction(() => document.querySelectorAll("dialog[open]").length === 0);
  const sheetPopoverClosedState = await page.evaluate(() => {
    ...
    const parentContent = document.querySelector("#runtime-sheet-popover-details-content");
    const nestedContent = document.querySelector("#runtime-sheet-popover-nested-content");
    const parentTrigger = document.querySelector("#runtime-sheet-popover-details-trigger");
    const nestedTrigger = document.querySelector("#runtime-sheet-popover-nested-trigger");
  ...
  if (
    ...
    throw new Error(
      `Expected closing Astro Sheet to reset open Popovers, got ${JSON.stringify(
        sheetPopoverClosedState,
      )}.`,
    );
  }

  await page.getByRole("button", { name: "Open sheet accordion menu" }).click();
  await page.getByRole("heading", { name: "Sheet accordion navigation" }).waitFor();
  const sheetAccordionInitialState = await page.evaluate(() => {
    const sheetContent = document.querySelector(
      "#runtime-sheet-accordion [data-slot='sheet-content']",
    );
    const root = document.querySelector("#runtime-sheet-accordion-menu");
    const sheetTrigger = document.querySelector("#runtime-sheet-accordion-trigger");
    const close = document.querySelector("#runtime-sheet-accordion-close");
    const productsTrigger = document.querySelector("#runtime-sheet-accordion-products-trigger");
    const productsContent = document.querySelector("#runtime-sheet-accordion-products-content");
    const servicesTrigger = document.querySelector("#runtime-sheet-accordion-services-trigger");
    const servicesContent = document.querySelector("#runtime-sheet-accordion-services-content");
    const nav = document.querySelector("#runtime-sheet-accordion nav");
  ...
  if (
    ...
    sheetAccordionInitialState.navLabel !== "Sheet accordion navigation" ||
    ...
    throw new Error(
      `Expected Astro Sheet accordion menu to initialize closed inside the sheet, got ${JSON.stringify(
  ...
  for (let index = 0; index < 10; index += 1) {
    ...
    if (activeId === "runtime-sheet-accordion-products-trigger") break;
  }
  if (!sheetAccordionProductsTabSequence.includes("runtime-sheet-accordion-products-trigger")) {
    throw new Error(
      `Expected Tab navigation to reach the Astro Sheet accordion Products trigger, got ${JSON.stringify(
  ...
  await page.keyboard.press("Enter");
  await page.waitForFunction(() => {
    const content = document.querySelector("#runtime-sheet-accordion-products-content");
    return (
      content instanceof HTMLElement &&
      !content.hidden &&
  ...
  if (
    ...
    sheetAccordionProductsState.activeElementId !== "runtime-sheet-accordion-products-trigger" ||
    ...
    throw new Error(
      `Expected keyboard opening Products in Astro Sheet accordion to keep the sheet open, got ${JSON.stringify(
  ...
  for (let index = 0; index < 10; index += 1) {
    ...
    if (activeId === "runtime-sheet-accordion-services-trigger") break;
  }
  if (!sheetAccordionServicesTabSequence.includes("runtime-sheet-accordion-services-trigger")) {
    throw new Error(
      `Expected Tab navigation to reach the Astro Sheet accordion Services trigger, got ${JSON.stringify(
  ...
  if (
    ...
    sheetAccordionServicesState.activeElementId !== "runtime-sheet-accordion-services-trigger" ||
    ...
    throw new Error(
      `Expected keyboard switching Services in Astro Sheet accordion to keep one open item, got ${JSON.stringify(
  ...
  if (
    ...
    sheetAccordionCollapsedState.activeElementId !== "runtime-sheet-accordion-services-trigger" ||
    ...
    throw new Error(
      `Expected pressing an open Astro Sheet accordion item to close it, got ${JSON.stringify(
  ...
  if (
    sheetAccordionClosedState.activeElementId !== "runtime-sheet-accordion-trigger" ||
    ...
    throw new Error(
      `Expected closing Astro Sheet accordion menu to hide the sheet and return focus, got ${JSON.stringify(
        sheetAccordionClosedState,
      )}.`,
    );
  }

  await page.getByRole("button", { name: "Open sheet FAQ" }).click();
  await page.getByRole("heading", { name: "Sheet FAQ accordion" }).waitFor();
  const sheetAccordionFaqInitialState = await page.evaluate(() => {
    const sheetContent = document.querySelector(
      "#runtime-sheet-accordion-faq [data-slot='sheet-content']",
    );
    const root = document.querySelector("#runtime-sheet-accordion-faq-menu");
    const availabilityTrigger = document.querySelector(
      "#runtime-sheet-accordion-faq-availability-trigger",
    );
    const availabilityContent = document.querySelector(
      "#runtime-sheet-accordion-faq-availability-content",
    );
  ...
  if (
    ...
    throw new Error(
      `Expected Astro Sheet FAQ accordion to start with a collapsible default-open item, got ${JSON.stringify(
        sheetAccordionFaqInitialState,
      )}.`,
    );
  }
  await page.locator("#runtime-sheet-accordion-faq-availability-trigger").click();
  await page.waitForFunction(() => {
    const content = document.querySelector("#runtime-sheet-accordion-faq-availability-content");
    return (
      content instanceof HTMLElement &&
      content.hidden &&
  ...
  if (
    ...
    throw new Error(
      `Expected Astro Sheet FAQ accordion default item to close in-place, got ${JSON.stringify(
