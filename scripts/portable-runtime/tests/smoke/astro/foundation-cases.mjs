export async function verifyAstroFoundationCases({ page }) {
  await verifyAstroProseCases({ page });
  ...
  if (
    ...
    buttonGroupState.dropdownTriggerSlot !== "button" ||
    ...
    throw new Error(
      `Expected Astro ButtonGroup demo to expose Starwind group, separator, text, input, and nested semantics, got ${JSON.stringify(
        buttonGroupState,
      )}.`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `Expected Astro Item demo to expose Item anatomy, variants, media, polymorphic root, and composed separator semantics, got ${JSON.stringify(
        itemState,
      )}.`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `Expected Astro Native Select demo to expose native select semantics, sizing, options, custom icon, and states, got ${JSON.stringify(
        nativeSelectState,
      )}.`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `Expected Astro Pagination demo to expose nav/list semantics, active page state, previous/next controls, ellipsis, and size variants, got ${JSON.stringify(
        paginationState,
      )}.`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `Expected Astro Table demo to expose semantic table parts, scroll wrapper, selected row state, and footer summary, got ${JSON.stringify(
        tableState,
      )}.`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `Expected Astro Aspect Ratio demo to expose wrapper ratio styles, image content, and polymorphic inner element, got ${JSON.stringify(
        aspectRatioState,
      )}.`,
    );
  }
  ...
  const buttonState = await page.evaluate(() => {
    const defaultButton = document.querySelector("#runtime-button-default");
    const submitButton = document.querySelector("#runtime-button-submit");
    const disabledButton = document.querySelector("#runtime-button-disabled");
    const focusableDisabledButton = document.querySelector("#runtime-button-focusable-disabled");
    const anchorButton = document.querySelector("#runtime-button-anchor");
    const ghostAnchorButton = document.querySelector("#runtime-button-anchor-ghost");
    ...
    return {
      anchorHref:
        anchorButton instanceof HTMLAnchorElement ? new URL(anchorButton.href).pathname : null,
      ...
      ghostAnchorHref:
        ghostAnchorButton instanceof HTMLAnchorElement
          ? new URL(ghostAnchorButton.href).pathname
          : null,
  ...
  const badgeState = await page.evaluate(() => {
    ...
    return {
      ...
      linkHref: linkBadge instanceof HTMLAnchorElement ? linkBadge.getAttribute("href") : null,
  ...
  if (
    ...
    throw new Error(
      `Expected Astro legacy badges with one styled link badge, got ${JSON.stringify(badgeState)}.`,
    );
  }

  const labelCount = await page.locator('[data-slot="label"][data-sw-label]').count();
  const smallLabelClass = await page
    .locator('label[data-slot="label"][for="runtime-label-sm"]')
    .getAttribute("class");
  const defaultLabelClass = await page
    .locator('label[data-slot="label"][for="runtime-label-md"]')
    .getAttribute("class");
  const largeLabelClass = await page
    .locator('label[data-slot="label"][for="runtime-label-lg"]')
    .getAttribute("class");
  const peerLabelClass = await page
    .locator('label[data-slot="label"][for="runtime-label-peer"]')
    .getAttribute("class");
  const labelAssociation = await page.evaluate(() => {
    const label = document.querySelector('label[data-slot="label"][for="runtime-label-sm"]');
  ...
  if (
    labelCount < 4 ||
    labelAssociation.activeId !== "runtime-label-sm" ||
    labelAssociation.controlId !== "runtime-label-sm" ||
    ...
    throw new Error(
      `Expected at least four Astro labels with native association and disabled styling, got ${JSON.stringify(
        {
          defaultLabelClass,
          disabledLabelStyle,
          labelCount,
  ...
  const defaultInput = page.locator("#runtime-input-default");
  const inputLabelAssociation = await page.evaluate(() => {
    const label = document.querySelector('label[data-slot="label"][for="runtime-input-default"]');
    if (label instanceof HTMLLabelElement) {
      label.click();
    }
  ...
  const invalidInputAttributes = await page.locator("#runtime-input-invalid").evaluate((input) => ({
    ariaInvalid: input.getAttribute("aria-invalid"),
    dataSlot: input.getAttribute("data-slot"),
    hasDataSwInput: input.hasAttribute("data-sw-input"),
  }));
  const disabledInputState = await page.locator("#runtime-input-disabled").evaluate((input) => {
  ...
  const fileInput = page.locator("#runtime-input-file");
  await fileInput.setInputFiles({
    buffer: Buffer.from("portable runtime"),
    mimeType: "text/plain",
    name: "runtime-note.txt",
  });
  ...
  await page.waitForFunction(() => {
    const defaultInput = document.querySelector("#runtime-input-default");
    const fileInput = document.querySelector("#runtime-input-file");
  ...
  if (
    inputCount < 4 ||
    inputLabelAssociation.activeId !== "runtime-input-default" ||
    inputLabelAssociation.controlId !== "runtime-input-default" ||
    ...
    initialInputState.name !== "runtime-input-default" ||
    ...
    invalidInputAttributes.ariaInvalid !== "true" ||
    invalidInputAttributes.dataSlot !== "input" ||
    invalidInputAttributes.hasDataSwInput !== true ||
    ...
    fileInputState.fileName !== "runtime-note.txt" ||
    fileInputState.formFileName !== "runtime-note.txt" ||
    ...
    throw new Error(
      `Expected at least four Astro inputs with runtime state, reset behavior, file support, and Starwind styling, got ${JSON.stringify(
        {
          defaultInputState,
          disabledInputState,
          fileInputState,
...
async function verifyAstroProseCases({ page }) {
  const proseState = await page.evaluate(() => {
    const root = document.querySelector("#runtime-prose-demo [data-slot='prose']");
    const heading = root?.querySelector("#runtime-prose-heading");
    const paragraph = root?.querySelector("#runtime-prose-paragraph");
    const code = root?.querySelector("#runtime-prose-code");
    const escapedCode = root?.querySelector("#runtime-prose-escaped-code");
    const escapedHeading = root?.querySelector("#runtime-prose-escaped-heading");
    const list = root?.querySelector("#runtime-prose-list");
  ...
  if (
    ...
    throw new Error(
      `Expected Astro Prose demo to render scoped sw-prose typography and preserve not-sw-prose escape styles, got ${JSON.stringify(
        proseState,
      )}.`,
    );
  }
