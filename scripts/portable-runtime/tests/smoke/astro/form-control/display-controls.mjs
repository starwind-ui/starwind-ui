export async function verifyAstroDisplayControlCases({ page }) {
  const textareaCount = await page
    .locator('textarea[data-slot="textarea"][data-sw-textarea]')
    .count();
  const defaultTextarea = page.locator("#runtime-textarea-default");
  const textareaLabelAssociation = await page.evaluate(() => {
    const label = document.querySelector(
      'label[data-slot="label"][for="runtime-textarea-default"]',
    );
  ...
  const invalidTextareaClass = await page
    .locator("#runtime-textarea-invalid")
    .getAttribute("class");
  ...
  const disabledTextareaStyle = await page
    .locator("#runtime-textarea-disabled")
    .evaluate((textarea) => {
  ...
  if (
    textareaCount !== 3 ||
    textareaLabelAssociation.activeId !== "runtime-textarea-default" ||
    textareaLabelAssociation.controlId !== "runtime-textarea-default" ||
    ...
    throw new Error(
      `Expected three Astro textareas with native input behavior and Starwind states, got ${JSON.stringify(
        {
  ...
  const skeletons = page.locator('#runtime-skeleton-demo [data-slot="skeleton"][data-sw-skeleton]');
  const skeletonCount = await skeletons.count();
  const skeletonStates = await page
    .locator('#runtime-skeleton-demo [data-slot="skeleton"][data-sw-skeleton]')
    .evaluateAll((items) =>
      items.map((item) => ({
  ...
  if (
    ...
    throw new Error(
      `Expected three Astro skeletons with Starwind classes and data markers, got ${JSON.stringify({
        skeletonCount,
        skeletonStates,
      })}.`,
    );
  ...
  if (
    ...
    throw new Error(
      `Expected Astro Card anatomy with Starwind classes and data markers, got ${JSON.stringify({
        cardCount,
        defaultCardState,
        cardPartStates,
        cardState,
  ...
  const breadcrumbState = await page.locator('nav[data-slot="breadcrumb"]').evaluate((nav) => {
    ...
    const asChild = nav.querySelector("[data-runtime-breadcrumb-as-child]");
  ...
  if (
    ...
    throw new Error(
      `Expected Astro Breadcrumb anatomy with Starwind classes and ARIA, got ${JSON.stringify(
        breadcrumbState,
      )}.`,
    );
  }
  ...
  const separatorState = await page.evaluate(() => {
    ...
    return {
      horizontal: readSeparator("#runtime-separator-horizontal"),
      vertical: readSeparator("#runtime-separator-vertical"),
    };
  });
  if (
    ...
    throw new Error(
      `Expected Astro Separator horizontal and vertical semantics, got ${JSON.stringify(
        separatorState,
      )}.`,
    );
  }
  ...
  const scrollAreaState = await page.evaluate(async () => {
    ...
    const defaultRoot = document.querySelector("#runtime-scroll-area-default");
    ...
    return {
      defaultArea: readScrollArea("#runtime-scroll-area-default"),
      ...
      twoAxisArea: readScrollArea("#runtime-scroll-area-both"),
    };
  });
  if (
    ...
    scrollAreaState.twoAxisArea.horizontalScrollbar.id !==
      "runtime-scroll-area-both-horizontal-scrollbar" ||
    ...
    scrollAreaState.twoAxisArea.verticalScrollbar.id !==
      "runtime-scroll-area-both-vertical-scrollbar" ||
    scrollAreaState.twoAxisArea.verticalScrollbar.display !== "flex" ||
    scrollAreaState.twoAxisArea.cornerDisplay !== "block"
  ) {
    throw new Error(
      `Expected Astro Scroll Area generated anatomy and overflow state, got ${JSON.stringify(
        scrollAreaState,
      )}.`,
    );
  }
