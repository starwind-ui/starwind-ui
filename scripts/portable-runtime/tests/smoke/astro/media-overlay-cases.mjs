...
export async function verifyAstroMediaOverlayCases({ page, serverMode = "preview" }) {
    ...
    const initial = {
      adversarial: Object.fromEntries(
        [
          ...
          "positive-infinity",
          "negative-infinity",
        ].map((name) => [name, readProgress(`runtime-progress-${name}`)]),
      ),
  ...
  for (const [name, expected] of Object.entries(expectedAdversarialProgress)) {
    ...
    if (JSON.stringify(received) !== JSON.stringify(expected)) {
      throw new Error(`Expected normalized Astro Progress ${name}, got ${JSON.stringify(actual)}.`);
    }
  }
  for (const name of ["nan", "positive-infinity", "negative-infinity"]) {
    ...
    if (
      ...
      throw new Error(
        `Expected indeterminate Astro Progress ${name}, got ${JSON.stringify(actual)}.`,
      );
    }
  }
